...
contract DeployImplementationV1 is Script {
    function run() public returns (ERC20Vault vault, RewardToken token, ERC1967Proxy proxy) {
        vm.startBroadcast();
        (vault, token, proxy) = deployCode();
        vm.stopBroadcast();
    }

    function deployCode() public returns (ERC20Vault, RewardToken, ERC1967Proxy) {
